...
def train(net,optimizer, train_data, train_labels, 
    ...
    #                         legend = legend) #可视动画    
            ...
            if train_step % 1 == 0:
                ...
                save_model(save_path + 'epoch_{}_loss_{:.5f}.pth'.format(epoch,l), optimizer, net, epoch)
    return train_loss
...
if __name__ == '__main__':
    ...
    lr_period = 30; lr_decay = 0.9
    epoch = 3
    Batch_Size = 40  
    save_path = 'F:\ZPY\github\\NTU_CRNet_withoutTrain\CRNet\model\\'
    
    #model
    if  os.path.exists('model/epoch_13_loss_0.00496.pth'):
        checkpoint = torch.load('model/epoch_38_loss_0.00319.pth',map_location='cpu')
    ...
    ##加载数据
    train_loader = get_dataloader('F:\ZPY\github\\NTU_CRNet_withoutTrain\CLT-GPR-dataset\Train', Batch_Size = Batch_Size)
    label_loader = get_dataloader('F:\ZPY\github\\NTU_CRNet_withoutTrain\CLT-GPR-dataset\Label', Batch_Size = Batch_Size)
